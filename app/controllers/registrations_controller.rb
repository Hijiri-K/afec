...
def response_to_sign_up_failure(resource)
  if resource.email == "" || resource.password == nil
    redirect_to root_path, notice: "Please fill in the form"
  elsif User.pluck(:email).include? resource.email
    redirect_to root_path, notice: "email already exists"
  elsif resource.password_confirmation !=  resource.password
    redirect_to root_path, notice: "password confirmation is wrong"
  else
    redirect_to root_path, notice: "password confirmation is wrong"
  end
end
